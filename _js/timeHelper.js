	...
	*/
	function enhanceTopicList(toEnhance, layer, parent){
		...
		for(var topic in toEnhance){
			...
			if(!currentlyEnhancing.color){
				currentlyEnhancing.color =  generatecolor();
			}
	...
	/*
	*	Generates a nice color. returns it in rgb(r, g, b) format as a string.
	*/
		...
		if(topic){
			...
			for(var i = 0; i < layerCount; i++){
				...
				if(topicHasSubElements){
					...
					progressBars[i].children().css('backgroundColor', loopingTopic.color);
					if(i != 0){
